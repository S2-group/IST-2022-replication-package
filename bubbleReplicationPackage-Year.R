  ...
  fileName <- paste("../paper/figures/Bubble", "ReplicationPackage-Year", ".png", sep="")
  
  # one element for each possible year
  yearsInt <- c(min(data_[[yearsCol]]):max(data_[[yearsCol]]))
  
  # possible values for the column
  labels <- c("yes/instructions_data_code", "no", "yes/data_code", "yes/only_code", "yes/only_data")
  shortLabels <- c("Instructions,\ncode & data", "None", "Code & data", "Code only", "Data only")
