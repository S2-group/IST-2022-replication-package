  ...
  Var1ColumnName <- "code_measured"
  Var2ColumnName <- "code_nrsites_grouped"
  fileName <- paste("../paper/figures/Bubble", "MainAspect-SubjectsNumber", ".png", sep="")
  Var1Label <- ""
  Var2Label <- ""
  bubbleScaler <- 1.25
  
  Var1OldValues = c("speed","enerconsum","bandwidth","cacheperform","memconsum")
  
  Var2ColumnOldValues <- c("<= 100", "<= 10", "not provided", "<= 50", "<= 1000", "> 1000")
  Var2ColumnNewValues <- c("<= 100", "<= 10", "Not\nprovided", "<= 50", "<= 1000", "> 1000")
  
  # name of the X and Y axis labels
  xLabel <- "Main aspect"
  yLabel <- "Subjects number"
  ...
  countOccurrences <- function(el1, el2) {
    ...
    for(i in 1:length(Elements1)) {
      ...
      if((grepl(el1, Elements1[i], fixed = TRUE)) && (el2 == Elements2[i])) {
        sum <- sum + 1
      }
    }
    return(sum)
  ...
  #Elements2 <- lapply(Elements2, function(vec) flatten_chr(strsplit(str_replace_all(vec, fixed(" "), ""), ",", perl = T)))
  #Elements2 <- flatten_chr(Elements2)
  
  #if (length(Elements2) > length(Elements1)){
  #   length(Elements1) = length(Elements2)
  #} else {
  ...
  #Reorder factors
  grid$Var2 <- factor(grid$Var2, levels = c("Not\nprovided", "<= 10", "<= 100", "<= 50", "<= 1000", "> 1000"))
  grid$Var1 <- factor(grid$Var1, levels = c("speed", "enerconsum", "bandwidth", "memconsum", "cacheperform"))
  ...
  p <- ggplot(grid,aes(Var1,Var2))+
    ...
    scale_x_discrete(labels = c("Performance", "Energy\nConsumption", "Bandwidth", "Memory\nConsumption",  "Caching")) + 
