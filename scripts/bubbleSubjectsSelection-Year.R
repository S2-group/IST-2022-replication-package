  ...
  columnName <- "code_sitesingle"
  
  #years column
  yearsCol <- "Year"
  
  fileName <- paste("../paper/figures/Bubble", "SubjectsSelection-Year", ".png", sep="")
  
  # one element for each possible year
  yearsInt <- c(min(data_[[yearsCol]]):max(data_[[yearsCol]]))
  
  # possible values for the column
  labels <- c("alexa", "nosource", "list", "other")
  shortLabels <- c("Alexa", "No source", "Ad-hoc\nlist", "Other")
