  ...
  columnName <- "code_sitestype_single"
  
  #years column
  yearsCol <- "Year"
  
  fileName <- paste("../paper/figures/Bubble", "SubjectsType-Year", ".png", sep="")
  
  # one element for each possible year
  yearsInt <- c(min(data_[[yearsCol]]):max(data_[[yearsCol]]))
  
  # possible values for the column
  labels <- c("real", "synthetic", "both")
  shortLabels <- c("Real", "Synthetic", "Both")
