  ...
  columnName <- "code_networkcondition_simple"
  
  #years column
  yearsCol <- "Year"
  
  fileName <- paste("../paper/figures/Bubble", "Network-Year", ".png", sep="")
  
  # one element for each possible year
  yearsInt <- c(min(data_[[yearsCol]]):max(data_[[yearsCol]]))
  
  # possible values for the column
  labels <- c("wifi", "not provided", "3g", "4g", "2g", "lte", "ethernet", "simulated", "GPRS", "not applicable")
  shortLabels <- c("WiFi", "Not provided", "3G", "4G", "2G", "LTE", "Ethernet", "Simulated", "GPRS", "Not applicable")
