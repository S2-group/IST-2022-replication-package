  ...
  columnName <- "code_nrsites_grouped"
  
  #years column
  yearsCol <- "Year"
  
  fileName <- paste("../paper/figures/Bubble", "SubjectsNumber-Year", ".png", sep="")
  
  # one element for each possible year
  yearsInt <- c(min(data_[[yearsCol]]):max(data_[[yearsCol]]))
  
  # possible values for the column
  labels <- c("not provided", "<= 10", "<= 50", "<= 100", "<= 1000", "> 1000")
  shortLabels <- c("Not provided", "<= 10", "<= 50", "<= 100", "<= 1000", "> 1000")
  ...
  grid$Type <- factor(grid$Type, levels = c("<= 10", "<= 50", "<= 100", "<= 1000", "> 1000", "Not provided"))
