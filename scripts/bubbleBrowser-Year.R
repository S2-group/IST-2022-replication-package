  ...
  columnName <- "code_browser_no_repetition"
  
  #years column
  yearsCol <- "Year"
  
  fileName <- paste("../paper/figures/Bubble", "Browser-Year", ".png", sep="")
  
  # one element for each possible year
  yearsInt <- c(min(data_[[yearsCol]]):max(data_[[yearsCol]]))
  
  # possible values for the column
  labels <- c("chrome", "modified", "firefox", "safari", "other", "not provided")
  shortLabels <- c("Chrome", "Modified", "FireFox", "Safari", "Other", "Not provided" )
