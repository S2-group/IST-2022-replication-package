  ...
  Var2ColumnName <- "code_dataanalysis"
  fileName <- paste("../paper/figures/Bubble", "MainAspect-Analysis", ".png", sep="")
  Var1Label <- ""
  Var2Label <- ""
  bubbleScaler <- 1.25
  ...
  Var2ColumnOldValues <- c("descstats", "predmod", "correlana", "hyptest", "effsize")
  Var2ColumnNewValues <- c("Descriptive\nstatistics", "Prediction\nmodels", "Correlation\nanalysis", "Hypothesis\ntesting", "Effect\nsize")
  
  # name of the X and Y axis labels
  xLabel <- "Main aspect"
  yLabel <- "Subjects type"
  ...
  countOccurrences <- function(el1, el2) {
    ...
    for(i in 1:length(Elements1)) {
      ...
      if((grepl( el1, Elements1[i], fixed = TRUE)) && (grepl(el2, Elements2[i], fixed = TRUE))) {
        sum <- sum + 1
      }
    }
    return(sum)
