  ...
  columnName <- "code_siteshosted"
  
  #years column
  yearsCol <- "Year"
  
  fileName <- paste("../paper/figures/Bubble", "Hosting-Year", ".png", sep="")
  
  # one element for each possible year
  yearsInt <- c(min(data_[[yearsCol]]):max(data_[[yearsCol]]))
  
  # possible values for the column
  labels <- c("original", "mirrored", "not provided")
  shortLabels <- c("Original", "Mirrored", "Not provided")
