  ...
  fileName <- paste("../paper/figures/Bubble", "MainAspect-Year", ".png", sep="")
  
  # one element for each possible year
  yearsInt <- c(min(data_[[yearsCol]]):max(data_[[yearsCol]]))
  
  # possible values for the column
  labels <- c("memconsum", "speed", "cacheperform", "bandwidth", "enerconsum")
  shortLabels <- c("Memory\nConsumption", "Performance", "Caching", "Bandwidth", "Energy\nCconsumption")
